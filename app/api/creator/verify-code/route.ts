...
<div style="max-width:500px;margin:40px auto;padding:0 20px;">
  ...
  <p style="color:#8a8580;font-size:12px;margin:0;">Ce code expire dans 10 minutes. Si vous n'avez pas demandé cette modification, ignorez cet email.</p>
</div></body></html>`
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Step 2: Verify code and apply change
    if (action === 'verify_and_change') {
      ...
      if (purpose === 'change_email' && newEmail) {
        const e1 = await sql`SELECT id FROM creators WHERE email = ${newEmail} AND id != ${session.id}`
        const e2 = await sql`SELECT id FROM restaurant_users WHERE email = ${newEmail}`
        ...
        await sql`UPDATE creators SET email = ${newEmail} WHERE id = ${session.id}`
        return NextResponse.json({ success: true })
      }
